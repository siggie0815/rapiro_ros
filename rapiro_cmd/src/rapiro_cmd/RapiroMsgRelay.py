...
class RapiroMsgRelay:
    ...
    def __init__(self):
        ...
        self.range_msg.field_of_view = 21.0 / 180.0 * math.pi
    ...
    def joint_state_cb(self, in_msg):
        ...
        self.joint_state_msg.position = [p / 180.0 * math.pi for p in bytearray(in_msg.pos)]
        self.joint_state_pub.publish(self.joint_state_msg)

    def trajectory_cb(self, traj_msg):
        if len(traj_msg.points):
            self.active_trajectory = traj_msg
            self.trajectory_msg = rapiro.Trajectory()
            self.trajectory_msg.stamp = self.active_trajectory.header.stamp
